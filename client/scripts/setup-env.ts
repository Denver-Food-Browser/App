 ...
 */
async function updateEnvVar(filePath: string, key: string, value: string): Promise<void> {
  let content = '';
...
async function updateEnvFile(filePath: string, updates: Record<string, string>): Promise<void> {
  for (const [key, value] of Object.entries(updates)) {
    await updateEnvVar(filePath, key, value);
  }
}
...
// Paths
const clientEnvPath = resolve(import.meta.dir, '../.env');
